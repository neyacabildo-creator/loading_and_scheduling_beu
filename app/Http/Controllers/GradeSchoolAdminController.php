...
class GradeSchoolAdminController extends Controller
{
    ...
    public function scheduleCreate() {
        ...
        if (!empty($gsSharedIds)) {
            ...
            foreach ($jhConflictRows as $r) {
                ...
                $teacherConflicts[$fid][] = [
                    ...
                    'start'   => substr($r->start_time, 0, 5),
                    'end'     => substr($r->end_time ?? '', 0, 5),
                    'section' => trim(($r->grade_level ?? '') . ' ' . ($r->section_name ?? '')) . ' (JH)',
                ];
            }
        }
        ...
        \App\Models\FacultyLoad::select('faculty_id', 'subject')
            ->whereNotNull('subject')->where('subject', '!=', '')
            ->get()
            ->each(function($fl) use (&$teachersBySubject, $gsSubjectNorm) {
                foreach (array_map('trim', explode(',', $fl->subject)) as $sub) {
                    if ($sub === '') continue;
                    $raw = strtoupper($sub);
                    $targets = $gsSubjectNorm[$raw] ?? [$raw];
        ...
        // Add shared teachers (with subjects assigned) to all GS grade levels
        // so they appear in the grade filter even before a subject is selected
        $gsGrades = ['Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5', 'Grade 6'];
        foreach ($sharedTeachers as $st) {
            if (!($st['faculty_id'] ?? null)) continue;
            $stSubjects = $st['subjects'] ?? null;
            if (is_string($stSubjects)) $stSubjects = json_decode($stSubjects, true);
            if (!is_array($stSubjects) || empty($stSubjects)) continue;
            foreach ($gsGrades as $grade) {
        ...
        // Build merged teacher list: regular GS teachers + active shared teachers
        $allTeachersForDropdown = $teachers->map(fn($t) => [
            'id'   => $t->id,
            'name' => trim(($t->first_name ?? '') . ' ' . ($t->last_name ?? '')) ?: ($t->name ?? ''),
        ])->values()->toArray();
        ...
        return view('grade-school-admin.schedule-form', compact(
            'teachers', 'rooms', 'teacherSubjects', 'teachersByGrade',
