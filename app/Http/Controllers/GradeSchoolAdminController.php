...
class GradeSchoolAdminController extends Controller
{
    ...
    public function getFacultyLoads(Request $request) {
        try {
            ...
            $approvedScheds = ClassSchedule::on('mysql_gs')->whereIn('faculty_id', $allFacultyIds)
                ->where('admin_approved', true)
                ->whereIn('status', ['active', 'approved'])
                ->get(['faculty_id', 'subject', 'section_name', 'day_of_week', 'start_time', 'end_time', 'grade_level'])
                ->groupBy('faculty_id');

            $result = $loads->map(function ($load) use ($users, $sharedTeacherIds, $approvedScheds) {
                $data = $load->toArray();
     ...
     */
    /**
     * Validate the create-schedule grid before POST (returns toast-friendly messages).
     */
    ...
    public function storeSchedule(Request $request) {
        ...
        try {
            ...
            $gsConflicts = \App\Support\ScheduleFormConflictSupport::collectGradeSchoolGridConflicts(
                $gradeLevel,
                $dayOfWeek,
                $scheduleDate,
                $slots,
