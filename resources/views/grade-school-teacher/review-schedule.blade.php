...
<style>
    ...
    .filter-group{display:flex;align-items:center;gap:.5rem;font-size:.875rem}
    .filter-group label{font-weight:600;color:var(--text-primary)}
    ...
    .btn-primary{background:var(--green-primary);color:white;padding:.5rem 1.25rem;border:none;border-radius:.375rem;cursor:pointer;font-weight:600;font-size:.875rem}
    ...
    .stat-box{background:var(--bg-secondary);padding:1rem 1.25rem;border-radius:.5rem;border:1px solid var(--border-color)}
    .stat-num{font-size:1.75rem;font-weight:bold;color:var(--green-primary)}
    .stat-lbl{font-size:.8rem;color:var(--text-secondary)}
...
<div class="stats-row">
    ...
    <div class="stat-box"><div class="stat-num" id="faculty-count">0</div><div class="stat-lbl">Faculty Members</div></div>
</div>
...
<div class="filter-card">
    ...
    <div class="filter-group">
        <label>Search:</label>
        <input type="text" id="filter-search" placeholder="Subject, faculty..." oninput="applyFilters()">
    </div>
    <button class="btn-primary" onclick="loadSchedules()" style="margin-left:auto"> Refresh</button>
</div>
...
async function loadSchedules() {
    try {
        const res = await fetch('/api/grade-school-teacher/schedules');
...
function updateStats(data) {
    ...
    document.getElementById('faculty-count').textContent = [...new Set(data.map(s => s.faculty_id))].length;
}
