...
@media print {
    ...
    <div class="fl-cards" id="fl-cards">
        <div class="fl-card">
    ...
    <div class="fl-meter-wrap" id="fl-meter" style="display:none;">
        <h2 class="fl-meter-title">
            ...
            Teaching Load Meter (vs. Standard 6 hrs/day)
        </h2>
        <div style="display:flex;align-items:center;gap:1rem;margin-bottom:.75rem;">
            ...
            <div style="flex:1;">
                ...
                <div class="fl-meter-labels">
                    ...
                    <span id="fl-meter-max">Standard: 30 hrs/week</span>
                </div>
            </div>
        </div>
...
<script>
@php $facultyLoadApi = $facultyLoadApi ?? '/api/teacher/faculty-load'; @endphp
(function () {
    const facultyLoadApi = @json($facultyLoadApi);
    const COLORS  = ['#2d7a50','#0369a1','#7c3aed','#b45309','#be185d','#0f766e','#c2410c','#1d4ed8','#15803d','#9333ea'];
    const DAYS    = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
    const MAX_HRS = 30; // standard weekly teaching load reference
    ...
    fetch(facultyLoadApi)
        ...
        .then(data => {
            ...
            const meterColor = pct > 100 ? '#dc2626' : pct >= 80 ? '#d97706' : '#2d7a50';
            text('fl-meter-pct',   pct + '%');
            text('fl-meter-used',  totalHrs.toFixed(1) + ' hrs used');
            const fill = document.getElementById('fl-meter-fill');
            if (fill) { fill.style.width = pct + '%'; fill.style.background = `linear-gradient(90deg,${meterColor},${meterColor}aa)`; }
            document.getElementById('fl-meter-pct').style.color = meterColor;
            text('fl-meter-msg', pct >= 100
                ? '⚠ You have reached or exceeded the standard weekly load. Please review with your administrator.'
                : pct >= 80
                ? 'Your load is approaching the standard limit. Plan additional tasks carefully.'
                : 'Your current teaching load is within the normal range.');
            show('fl-meter');
            ...
            schedules.forEach(s => {
                ...
                if (d && dayMap[d]) { dayMap[d].count++; dayMap[d].hrs += parseFloat(s.load_hours || 0); }
            });
            const activeDaysList = DAYS.filter(d => dayMap[d].count > 0);
            if (activeDaysList.length) {
                document.getElementById('fl-days').innerHTML = activeDaysList.map(d => `
                ...
                <div class="fl-subj-card">
                    ...
                    <div class="fl-subj-body">
                        ...
                        <div>
                            ...
                            <p class="fl-subj-meta-val" style="font-size:.78rem;">${timeStr}</p>
                        </div>
                        <div>
