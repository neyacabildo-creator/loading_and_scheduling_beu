...
@section('content')
    ...
    <div class="form-wrapper">
        <div>
            <a href="{{ route('grade-school-admin.faculty-loading') }}" class="back-link">← Back to Faculty Loading</a>
            ...
            <div class="form-card">
                ...
                <form method="POST" action="{{ $facultyLoad ? route('grade-school-admin.faculty-loading.update', $facultyLoad) : route('grade-school-admin.faculty-loading.store') }}">
                    @csrf
                    @if($facultyLoad)
                        @method('PUT')
                    @endif
                    ...
                    <script type="application/json" id="teachers-data">@json($teachers->map(fn($t) => ['id' => $t->id, 'name' => ($t->first_name ?? '') . ' ' . ($t->last_name ?? ''), 'email' => $t->email, 'is_shared' => in_array((string)$t->id, $sharedTeacherUserIds ?? [])]))</script>
                    ...
                    <div class="form-group">
                        ...
                        <select id="gsGradeLevelSelect" name="grade_level" class="form-select @error('grade_level') error-input @enderror">
                            <option value="">-- Select Grade Level --</option>
                            @foreach(['Grade 1','Grade 2','Grade 3','Grade 4','Grade 5','Grade 6'] as $grade)
                    ...
                    <div class="form-group">
                        ...
                        <select id="gsSubjectSelect" name="subject" class="form-select @error('subject') error-input @enderror">
                    ...
                    <div class="button-group">
                        ...
                        <a href="{{ route('grade-school-admin.faculty-loading') }}" class="btn-cancel">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
    // Grade level → Subject mapping for GS
    ...
    const gsGradeEl   = document.getElementById('gsGradeLevelSelect');
    const gsSubjectEl = document.getElementById('gsSubjectSelect');
    const gsCurrentSubject = @json(old('subject', $facultyLoad?->subject ?? ''));

    function gsPopulateSubjects(grade) {
        const subjects = GS_GRADE_SUBJECTS[grade] || Object.values(GS_GRADE_SUBJECTS).flat().filter((v,i,a)=>a.indexOf(v)===i).sort();
        gsSubjectEl.innerHTML = '<option value="">-- Select Subject --</option>'
            + subjects.map(s => `<option value="${s}"${s === gsCurrentSubject ? ' selected' : ''}>${s}</option>`).join('');
    }

    gsGradeEl.addEventListener('change', () => gsPopulateSubjects(gsGradeEl.value));
    gsPopulateSubjects(gsGradeEl.value);
    </script>
@endsection
