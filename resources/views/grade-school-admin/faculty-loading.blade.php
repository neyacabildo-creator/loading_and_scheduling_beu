...
@section('content')
    ...
    <script>
                ...
                return `<tr>
                    ...
                    <td>
                        <div style="display:flex;align-items:center;gap:0.4rem;flex-wrap:nowrap;">
                            ...
                            ${load.faculty_id && load.has_user_account ? (GS_KINDER_GRADES.includes(load.grade_level) ? `<button type="button" onclick="openScheduleCard(${load.faculty_id}, '${String(load.grade_level || '').replace(/'/g, "\\'")}')" style="padding:0.4rem 0.75rem;border:1px solid #4338ca;border-radius:0.375rem;cursor:pointer;font-size:0.75rem;font-weight:500;background:#4338ca;color:white;white-space:nowrap;">Kinder Schedule</button>` : `<a href="${masterScheduleBaseUrl}/${load.faculty_id}" style="padding:0.4rem 0.75rem;border:1px solid var(--green-primary);border-radius:0.375rem;font-size:0.75rem;font-weight:500;background:var(--green-primary);color:white;text-decoration:none;display:inline-block;white-space:nowrap;">Schedule</a><button type="button" onclick="openScheduleCard(${load.faculty_id}, '${String(load.grade_level || '').replace(/'/g, "\\'")}')" style="padding:0.4rem 0.75rem;border:1px solid var(--green-primary);border-radius:0.375rem;cursor:pointer;font-size:0.75rem;font-weight:500;background:transparent;color:var(--green-primary);white-space:nowrap;">View Card</button>`) : ''}
                        </div>
                    </td>
                </tr>`;
            }).join('');
        ...
        let scheduleCardPhotoName = 'schedule-card';
        ...
        function openScheduleCard(teacherId, gradeLevel) {
            ...
            scheduleCardPhotoName = 'schedule-card-' + teacherId;
            if (gradeLevel && GS_KINDER_GRADES.includes(gradeLevel)) {
                cardUrl += '&grade_level=' + encodeURIComponent(gradeLevel);
                scheduleCardPhotoName = 'kinder-schedule-' + teacherId + '-' + (gradeLevel || 'kinder').replace(/\s+/g, '-');
            } else {
            ...
            .then(d => {
                ...
                depts.forEach(dept => {
                    ...
                    teachers.forEach(t => {
                        ...
                        const gsLine = t.gs_subjects !== '—' ? '<br><span style="font-size:.7rem;font-weight:400;color:#10b981;">GS: ' + t.gs_subjects + '</span>' : '';
        ...
        <div style="background:#fff;border-radius:.75rem;width:96%;max-width:1000px;max-height:93vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 10px 40px rgba(0,0,0,.35);">
            ...
            <div style="padding:.75rem 1rem;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #e5e7eb;flex-shrink:0;">
                ...
                <div style="display:flex;gap:.5rem;align-items:center;">
                    <button id="scheduleCardDownloadBtn" type="button" onclick="downloadScheduleCardPhoto()"
