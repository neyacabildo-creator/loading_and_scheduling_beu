...
@section('content')
    ...
    <div class="header">
        ...
        <div class="header-right">
            <a href="{{ route('grade-school-admin.users.index') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.5rem; background: transparent; color: var(--text-primary); border: 1px solid var(--border-color); border-radius: 0.5rem; text-decoration: none; font-weight: 600; font-size: 0.875rem;">
                ← Back to Users
            </a>
        </div>
    </div>
    ...
    <div class="table-card" style="max-width: 600px;">
        ...
        <form style="padding: 2rem;" action="{{ route('grade-school-admin.users.store') }}" method="POST">
            @csrf
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="role_id" id="gs_role_id" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.5rem; background: var(--bg-secondary); color: var(--text-primary);" required onchange="toggleSharedSubjectsGS(this.value)">
            ...
            <div id="gs_shared_subjects_wrap" style="display:none; margin-bottom: 1.5rem; background: rgba(59,130,246,.06); border: 1px solid rgba(59,130,246,.2); border-radius: 0.5rem; padding: 1rem;">
                ...
                <div style="margin-bottom:0.75rem;">
                    ...
                    <select name="subject1" id="gs_subject1" style="width:100%; padding:0.65rem; border:1px solid var(--border-color); border-radius:0.5rem; background:var(--bg-secondary); color:var(--text-primary);">
                ...
                <div>
                    ...
                    <select name="subject2" id="gs_subject2" style="width:100%; padding:0.65rem; border:1px solid var(--border-color); border-radius:0.5rem; background:var(--bg-secondary); color:var(--text-primary);">
            ...
            <div style="display: flex; gap: 1rem;">
                ...
                <a href="{{ route('grade-school-admin.users.index') }}" style="flex: 1; padding: 0.75rem; background: var(--bg-primary); color: var(--text-primary); border: 1px solid var(--border-color); border-radius: 0.5rem; cursor: pointer; font-weight: 600; text-align: center; text-decoration: none;">Cancel</a>
            </div>
        </form>
    </div>
@endsection
...
@push('scripts')
<script>
function toggleSharedSubjectsGS(roleId) {
    // Role ID 7 = Shared Teacher
    var wrap = document.getElementById('gs_shared_subjects_wrap');
    if (wrap) wrap.style.display = (String(roleId) === '7') ? 'block' : 'none';
    var s1 = document.getElementById('gs_subject1');
    if (s1) s1.required = (String(roleId) === '7');
}
(function() {
    var roleId = document.getElementById('gs_role_id').value;
    if (roleId) toggleSharedSubjectsGS(roleId);
})();
</script>
@endpush
