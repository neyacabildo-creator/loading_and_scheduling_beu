{{-- resources/views/grade-school-admin/schedule-approval/index.blade.php --}}
...
@section('content')
    <style>
        ...
        html[data-theme="dark"] .status-card { background: var(--bg-secondary); border-color: var(--border-color); }
    ...
    <div class="table-card">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $schedule->room_id ? 'Room #'.$schedule->room_id : 'N/A' }}</td>
                        ...
                        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;align-items:center;">
                                @if($schedule->status !== 'active' || !$schedule->admin_approved)
                                    <button class="action-btn btn-approve"
                                        data-id="{{ $schedule->id }}"
                                        onclick="gsApprove(this.dataset.id, this)">Approve</button>
                                @endif
                                @if($schedule->status !== 'rejected')
                                    <button class="action-btn btn-reject"
                                        data-id="{{ $schedule->id }}"
                                        onclick="gsReject(this.dataset.id, this)">Reject</button>
                                @endif
                                <button class="action-btn btn-view"
                                    data-id="{{ $schedule->id }}"
                                    onclick="gsDelete(this.dataset.id, this)"
                                    style="background:rgba(239,68,68,0.1);color:#ef4444;"
                                    title="Delete schedule">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
        ...
        @else
        <div class="empty-state"><p>No schedules found.</p></div>
        @endif
    </div>
    ...
    <div class="calendar-card">
        <div class="calendar-header">
            ...
            <div style="display:flex;align-items:center;gap:1rem;">
                <span id="gsApprovalMonthYear" style="font-weight:600;color:var(--text-primary);min-width:150px;text-align:center;"></span>
                <div class="month-nav">
                    <button class="month-btn" onclick="gsPrevMonth()">‹ Previous</button>
                    <button class="month-btn" onclick="gsToday()">Today</button>
                    <button class="month-btn" onclick="gsNextMonth()">Next ›</button>
                </div>
            </div>
        </div>
        <div class="weekdays">
        ...
        <div class="calendar-grid" id="gs-approval-calendar-days"></div>
    </div>

    <script>
        const _csrf = document.querySelector('meta[name="csrf-token"]')?.content || '';
        let gsCalDate    = new Date();
        let gsSchedules  = [];

        function renderGSCalendar() {
            const year  = gsCalDate.getFullYear();
            const month = gsCalDate.getMonth();

            document.getElementById('gsApprovalMonthYear').textContent =
                gsCalDate.toLocaleString('default', { month: 'long', year: 'numeric' });
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                const dateDow = dayNames[new Date(dateStr + 'T00:00:00').getDay()];

                const dayScheds = gsSchedules.filter(s =>
                ...
                html += `<div class="calendar-day ${isToday ? 'today' : ''}">
                    ...
                    ${dayScheds.slice(0, 2).map(s => {
                        const cls = s.admin_approved ? '' : 'pending';
            ...
            document.getElementById('gs-approval-calendar-days').innerHTML = html;
        }

        function gsPrevMonth() { gsCalDate.setMonth(gsCalDate.getMonth() - 1); renderGSCalendar(); }
        function gsNextMonth() { gsCalDate.setMonth(gsCalDate.getMonth() + 1); renderGSCalendar(); }
        function gsToday()     { gsCalDate = new Date(); renderGSCalendar(); }

        function loadGSCalendarSchedules() {
            fetch('/api/admin/schedules', {
                headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': _csrf },
                credentials: 'same-origin'
            })
            .then(r => r.json())
            .then(data => { gsSchedules = data.data || data || []; renderGSCalendar(); })
            .catch(() => renderGSCalendar());
        }

        function gsApprove(id, btn) {
            if (!confirm('Approve this schedule?')) return;
            btn.disabled = true;
            fetch(`/api/admin/schedules/${id}/approve`, {
                method: 'POST',
            ...
            .then(r => r.json())
            .then(d => {
                if (d.success !== false) { location.reload(); }
                else { alert('Error: ' + (d.message || 'Failed to approve')); btn.disabled = false; }
            })
            .catch(() => { alert('Error approving schedule.'); btn.disabled = false; });
        }

        function gsReject(id, btn) {
            ...
            .then(d => {
                if (d.success !== false) { location.reload(); }
                else { alert('Error: ' + (d.message || 'Failed to reject')); btn.disabled = false; }
            })
            .catch(() => { alert('Error rejecting schedule.'); btn.disabled = false; });
        }

        function gsDelete(id, btn) {
            ...
            .catch(() => { alert('Error deleting schedule.'); btn.disabled = false; });
        }

        document.addEventListener('DOMContentLoaded', function () {
            renderGSCalendar();
            loadGSCalendarSchedules();
            setInterval(loadGSCalendarSchedules, 30000);
        });

        window.addEventListener('pageshow', function(event) {
            if (event.persisted) { loadGSCalendarSchedules(); }
        });
    </script>
@endsection
