...
@section('content')
    <style>
        ...
        /* Dark mode overrides */
        html[data-theme="dark"] .settings-nav { background: var(--bg-secondary); border-color: var(--border-color); }
        html[data-theme="dark"] .settings-nav-item { color: var(--text-primary) !important; }
        html[data-theme="dark"] .settings-nav-item:hover { background: var(--bg-tertiary); color: var(--text-primary); }
        html[data-theme="dark"] .settings-nav-item.active { background: rgba(74, 157, 111, 0.15); color: var(--green-primary); }
        html[data-theme="dark"] .settings-content { background: var(--bg-secondary); border-color: var(--border-color); }
        html[data-theme="dark"] .setting-section { border-color: var(--border-color); }
        html[data-theme="dark"] .section-title,
        html[data-theme="dark"] .setting-label { color: var(--text-primary); }
        html[data-theme="dark"] .setting-description { color: var(--text-secondary); }
        html[data-theme="dark"] .setting-input { background: var(--bg-tertiary); border-color: var(--border-color); color: var(--text-primary); }
        html[data-theme="dark"] .btn-secondary { background: var(--bg-secondary); border-color: var(--border-color); color: var(--text-primary); }
        html[data-theme="dark"] .btn-secondary:hover { border-color: var(--green-primary); color: var(--green-primary); }
        html[data-theme="dark"] .info-box { background: rgba(12, 74, 110, 0.25); border-color: rgba(96, 165, 250, 0.3); color: #93c5fd; }
        html[data-theme="dark"] [style*="background: #f5f3ed"] { background: var(--bg-tertiary) !important; }
        html[data-theme="dark"] [style*="color: #2d3436"] { color: var(--text-primary) !important; }
        html[data-theme="dark"] [style*="color: #7a7a6e"] { color: var(--text-secondary) !important; }
        html[data-theme="dark"] [style*="border-bottom: 1px solid #e8dcc8"] { border-color: var(--border-color) !important; }
    </style>
            ...
            <div id="backup" class="tab-content" style="display: none;">
                ...
                <div class="setting-section">
                    ...
                    <p class="setting-description" style="margin-bottom:1rem;">
                        ...
                        plus faculty load data for the Junior High School department.
                    </p>
                    <a href="{{ route('admin.backup.download') }}" class="btn btn-primary" style="display:inline-block;text-decoration:none;">
                ...
                <div class="setting-section">
                    ...
                    <div style="display:flex;align-items:center;gap:2rem;margin-bottom:1.5rem;">
                        <div id="photoPreviewWrap" style="width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#2d7a50,#1a5336);display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;">
                            ...
                            @else
                                <span id="photoPreview" style="color:white;font-size:1.5rem;font-weight:700;display:flex;align-items:center;justify-content:center;width:100%;height:100%;">{{ strtoupper(substr(Auth::user()->first_name, 0, 1)) }}</span>
                            @endif
                        </div>
                        <form method="POST" action="{{ route('admin.profile.photo') }}" enctype="multipart/form-data" style="flex:1;">
                            @csrf
                            <div class="setting-group">
    ...
    <script>
        ...
        function switchTab(tabName) {
            ...
            // Toggle global save button (hide for profile & backup â€” those have their own actions)
            document.getElementById('save-btn-group').style.display = (tabName === 'profile' || tabName === 'backup') ? 'none' : 'flex';

            // Update nav active â€” use attribute selector to avoid relying on event.target
