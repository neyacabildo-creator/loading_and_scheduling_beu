...
@section('content')
    <style>
        ...
        .modal-err-banner { display: none; background: rgba(200,50,50,0.1); border: 1px solid #c83232; color: #c83232; padding: 0.75rem 1rem; border-radius: 0.5rem; margin-bottom: 1.25rem; font-size: 0.875rem; }
    ...
    <!-- Add User Modal -->
    <div id="addUserModal" class="modal-overlay" onclick="handleModalOverlay(event)">
        <div class="modal-box" id="modalBox">
                ...
                <div id="m_subjects_container" style="display:none;">
                    <div class="form-row">
                        <div class="form-field">
                            ...
                            <select id="m_subject1" class="form-input">
                                <option value="">— Select Subject —</option>
                            </select>
                            <span class="field-error" id="err_subject1"></span>
                        </div>
                        <div class="form-field">
                            <label class="form-label">Secondary Subject <span style="color:#c83232">*</span></label>
                            <select id="m_subject2" class="form-input">
                                <option value="">— Select Subject —</option>
                            </select>
                            <span class="field-error" id="err_subject2"></span>
                        </div>
                    </div>
                </div>
                <div class="form-field">
            ...
            <div class="modal-foot">
                ...
                <button class="btn-modal-submit" id="submitUserBtn" onclick="submitAddUser()">Create User</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function populateSubjectSelects() {
            const s1 = document.getElementById('m_subject1');
            const s2 = document.getElementById('m_subject2');
            const defaultOption = '<option value="">— Select Subject —</option>';
            const options = jhSubjects.map(s => `<option value="${s}">${s}</option>`).join('');
        ...
        function toggleActive(id, currentlyActive) {
            ...
            btn.disabled = true;
            btn.textContent = '...';
            fetch('/api/teachers/' + id + '/toggle-active', {
                method: 'PATCH',
                headers: {
        ...
        /* ── Modal ── */
        const MODAL_FIELDS = ['first_name', 'last_name', 'email', 'role_id', 'password', 'password_confirmation'];
        ...
        function clearModalForm() {
            ...
            const banner = document.getElementById('modalErrBanner');
        ...
        function submitAddUser() {
            ...
            MODAL_FIELDS.forEach(f => setFieldError(f, ''));
            document.getElementById('modalErrBanner').style.display = 'none';
            ...
            .then(json => {
                if (json.success) {
                    ...
                    MODAL_FIELDS.forEach(f => setFieldError(f, (errors[f] || [])[0] || ''));
        ...
        function showCredentialsCard(name, email, password) {
            document.getElementById('jh_cred_name').textContent  = name;
            document.getElementById('jh_cred_email').textContent = email;
            document.getElementById('jh_cred_pwd').textContent   = password;
            document.getElementById('jhCredCard').style.display  = 'flex';
        }

        function closeCredCard() {
            document.getElementById('jhCredCard').style.display = 'none';
        }

        function copyCredentials() {
            const name  = document.getElementById('jh_cred_name').textContent;
            const email = document.getElementById('jh_cred_email').textContent;
            const pwd   = document.getElementById('jh_cred_pwd').textContent;
            const text  = 'Account Credentials\nName: ' + name + '\nEmail: ' + email + '\nPassword: ' + pwd;
            navigator.clipboard.writeText(text).then(function() {
                const btn = document.getElementById('jhCopyCredsBtn');
        ...
        function openEditModal(id) {
            ...
            setTimeout(() => document.getElementById('edit_first_name').focus(), 50);
        }
        ...
        function clearEditErrors() {
            ['first_name','last_name','email'].forEach(f => {
                ...
                const err = document.getElementById('edit_err_' + f);
                if (err) err.textContent = '';
            });
            const banner = document.getElementById('editErrBanner');
            banner.style.display = 'none';
            banner.textContent = '';
        }
                ...
                if (json.success) {
                    ...
                    ['first_name','last_name','email'].forEach(f => {
                        ...
                        if (msg) {
                            ...
                            const errEl = document.getElementById('edit_err_' + f);
                            if (errEl) errEl.textContent = msg;
                        }
                    });
                    if (json.message && !Object.keys(errors).length) {
    ...
    <div id="editUserModal" class="modal-overlay" onclick="if(event.target===this)closeEditModal()">
        <div class="modal-box">
            ...
            <div class="modal-body">
                <div id="editErrBanner" class="modal-err-banner"></div>
                <input type="hidden" id="edit_id">
                <div class="form-row">
                    <div class="form-field">
                        <label class="form-label">First Name</label>
                        <input type="text" id="edit_first_name" class="form-input" placeholder="First name">
                        <span class="field-error" id="edit_err_first_name"></span>
                    </div>
                    <div class="form-field">
                        <label class="form-label">Last Name</label>
                        <input type="text" id="edit_last_name" class="form-input" placeholder="Last name">
                        <span class="field-error" id="edit_err_last_name"></span>
                    </div>
                </div>
                <div class="form-field" style="margin-bottom:0;">
                    <label class="form-label">Email Address</label>
                    <input type="email" id="edit_email" class="form-input" placeholder="email@example.com">
                    <span class="field-error" id="edit_err_email"></span>
                </div>
            </div>
            <div class="modal-foot">
                <button class="btn-modal-cancel" onclick="closeEditModal()">Cancel</button>
                <button class="btn-modal-submit" id="editUserBtn" onclick="submitEditUser()">Save Changes</button>
            </div>
        </div>
    </div>

    {{-- Credential Card — shown once after account creation so admin can share with instructor --}}
    <div id="jhCredCard" style="display:none;position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.6);align-items:center;justify-content:center;">
        <div style="background:#fff;border-radius:.75rem;width:100%;max-width:400px;margin:1rem;padding:2rem;box-shadow:0 24px 64px rgba(0,0,0,.3);">
            ...
            <div style="background:#f5f3ed;border-radius:.5rem;padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;flex-direction:column;gap:.75rem;">
                <div>
                    ...
                    <div id="jh_cred_name" style="font-weight:600;color:#2d3436;"></div>
                </div>
                <div>
                    ...
                    <div id="jh_cred_email" style="color:#2d3436;word-break:break-all;"></div>
                </div>
                <div>
                    ...
                    <div id="jh_cred_pwd" style="font-weight:700;font-family:monospace;font-size:1.05rem;color:#1a5336;letter-spacing:.04em;"></div>
                </div>
            </div>
            <div style="display:flex;flex-direction:column;gap:.6rem;">
                <button id="jhCopyCredsBtn" onclick="copyCredentials()" style="width:100%;padding:.75rem;background:linear-gradient(135deg,#2d7a50,#1a5336);color:#fff;border:none;border-radius:.5rem;font-weight:600;cursor:pointer;font-size:.875rem;">Copy Credentials</button>
