{{-- resources/views/junior-high-teacher/loading-schedule.blade.php --}}
...
.ls-day-bar  { height:100%; background:linear-gradient(90deg,#2d7a50,#4ade80); border-radius:9999px; min-width:4px; transition:width .5s; }
...
@media print {
    ...
    <div style="background:linear-gradient(135deg,#1a5336 0%,#2d7a50 60%,#3d9970 100%);border-radius:.75rem;padding:2rem;margin-bottom:2rem;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
        <div>
            <h1 style="color:white;font-size:1.75rem;font-weight:800;margin:0 0 .3rem;">My Loading Schedule</h1>
            <p style="color:rgba(255,255,255,.75);font-size:.875rem;margin:0;">Your assigned subjects and schedule for the current semester</p>
        </div>
...
@php
    /* Colour palette – one per unique subject */
...
@endphp
    ...
    <div class="ls-days">
        ...
        @foreach($dayTotals as $day => $info)
            ...
            @endif
        @endforeach
        @if($schedules->whereNull('day_of_week')->merge($schedules->where('day_of_week',''))->count())
        <p style="font-size:.78rem;color:var(--text-secondary);margin:.5rem 0 0;">
            {{ $schedules->filter(fn($s)=>!$s->day_of_week)->count() }} subject(s) do not have a day assigned yet.
        </p>
        @endif
    </div>
    @endif
    ...
    <p style="text-align:center;color:var(--text-secondary);font-size:.8rem;margin:1.5rem 0;">
        Junior High School Teacher Portal &mdash; Loading schedule data is managed by your department administrator.
    ...
    async function submitAdjRequest() {
        ...
        try {
            ...
            if (json.success) {
                ...
                alert('Error: ' + (json.error ?? json.message ?? 'Could not submit request.'));
            }
        } catch (err) {
