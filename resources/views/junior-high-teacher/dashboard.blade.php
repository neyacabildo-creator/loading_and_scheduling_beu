{{-- resources/views/junior-high-teacher/dashboard.blade.php --}}
@extends('layouts.teacher')

@section('title', 'Teacher Dashboard - Junior High School')
...
@section('content')
    ...
    <div style="background:linear-gradient(135deg,#1a5336 0%,#2d7a50 60%,#3d9970 100%);border-radius:.75rem;padding:2rem;margin-bottom:2rem;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
        <div>
            <p style="color:rgba(255,255,255,.8);font-size:.85rem;margin-bottom:.35rem;">Good <span id="jhGreeting">day</span></p>
            <h1 style="color:white;font-size:1.75rem;font-weight:800;margin:0 0 .25rem;">{{ auth()->user()->first_name ?? auth()->user()->name ?? 'Teacher' }}</h1>
            <p style="color:rgba(255,255,255,.75);font-size:.875rem;margin:0;">Junior High School Division &bull; <span id="jhLiveDate"></span></p>
        </div>
    ...
    (function(){
        ...
        document.getElementById('jhGreeting').textContent = h < 12 ? 'morning' : h < 17 ? 'afternoon' : 'evening';
        document.getElementById('jhLiveDate').textContent = new Date().toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'long',day:'numeric'});
    })();
    </script>
    ...
    <div style="background:var(--bg-secondary);border-radius:.75rem;padding:1.25rem 1.5rem;border:1px solid var(--border-color);margin-bottom:1.5rem;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:.75rem;">
            <a href="{{ route('teacher.class-schedule') }}" style="display:flex;align-items:center;gap:.6rem;padding:.7rem .9rem;background:rgba(45,122,80,.07);border:1px solid rgba(45,122,80,.2);border-radius:.5rem;text-decoration:none;color:var(--text-primary);">
            ...
            <a href="{{ route('teacher.review-schedule') }}" style="display:flex;align-items:center;gap:.6rem;padding:.7rem .9rem;background:rgba(45,122,80,.07);border:1px solid rgba(45,122,80,.2);border-radius:.5rem;text-decoration:none;color:var(--text-primary);">
            ...
            <a href="{{ route('teacher.feedback') }}" style="display:flex;align-items:center;gap:.6rem;padding:.7rem .9rem;background:rgba(45,122,80,.07);border:1px solid rgba(45,122,80,.2);border-radius:.5rem;text-decoration:none;color:var(--text-primary);">
            ...
            <a href="{{ route('teacher.request-adjustments') }}" style="display:flex;align-items:center;gap:.6rem;padding:.7rem .9rem;background:rgba(45,122,80,.07);border:1px solid rgba(45,122,80,.2);border-radius:.5rem;text-decoration:none;color:var(--text-primary);">
            ...
            <a href="{{ route('teacher.faculty-loading') }}" style="display:flex;align-items:center;gap:.6rem;padding:.7rem .9rem;background:rgba(45,122,80,.07);border:1px solid rgba(45,122,80,.2);border-radius:.5rem;text-decoration:none;color:var(--text-primary);">
            ...
            <a href="{{ route('teacher.print-export') }}" style="display:flex;align-items:center;gap:.6rem;padding:.7rem .9rem;background:rgba(45,122,80,.07);border:1px solid rgba(45,122,80,.2);border-radius:.5rem;text-decoration:none;color:var(--text-primary);">
            ...
            <a href="{{ route('teacher.loading-schedule') }}" style="display:flex;align-items:center;gap:.6rem;padding:.7rem .9rem;background:rgba(45,122,80,.07);border:1px solid rgba(45,122,80,.2);border-radius:.5rem;text-decoration:none;color:var(--text-primary);">
                ...
                <div style="display:flex;flex-direction:column;gap:.75rem;">
                    ...
                    <div style="display:flex;align-items:center;gap:.875rem;padding:.75rem;background:{{ $isNow ? 'rgba(45,122,80,.12)' : 'var(--bg-primary)' }};border-radius:.5rem;border-left:3px solid {{ $isNow ? '#2d7a50' : 'var(--border-color)' }};">
                        ...
                        <div style="text-align:right;flex-shrink:0;">
                            ...
                            @else
                                <span style="background:var(--bg-primary);border:1px solid var(--border-color);color:var(--text-secondary);font-size:.65rem;padding:.2rem .55rem;border-radius:2rem;white-space:nowrap;">{{ $sch->room_id ? ($sch->room->room_number ?? 'Room #'.$sch->room_id) : (trim(($sch->grade_level ?? '') . ($sch->section_name ? ' – '.$sch->section_name : '')) ?: 'TBA') }}</span>
                            @endif
                        </div>
                    </div>
                    @endforeach
    ...
    <div style="background:var(--bg-secondary);border-radius:.75rem;border:1px solid var(--border-color);overflow:hidden;margin-bottom:1.5rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:1.25rem 1.5rem;border-bottom:1px solid var(--border-color);">
            ...
            <a href="{{ route('teacher.review-schedule') }}" style="padding:.45rem .9rem;background:#2d7a50;color:#fff;border-radius:.375rem;font-size:.78rem;font-weight:600;text-decoration:none;">Review</a>
        </div>
                ...
                <table style="width:100%;border-collapse:collapse;">
                    ...
                    <tbody>
                        ...
                        <tr style="border-top:1px solid var(--border-color);font-size:.875rem;">
                            ...
                            <td style="padding:.75rem 1rem;color:var(--text-secondary);">{{ $sch->room_id ? ($sch->room->room_number ?? 'Room #'.$sch->room_id) : (trim(($sch->grade_level ?? '') . ($sch->section_name ? ' – '.$sch->section_name : '')) ?: 'TBA') }}</td>
    ...
    {{-- ── STL Section (Subject Team Leaders only) ────────────────── --}}
    @if($isSTL ?? false)
    <div style="background:var(--bg-secondary);border-radius:.75rem;padding:1.5rem;border:1px solid var(--border-color);margin-bottom:1.5rem;">
        <h3 style="font-size:1rem;font-weight:700;color:var(--text-primary);margin:0 0 1rem;">Subject Team Leadership</h3>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:.75rem;margin-bottom:1.25rem;">
            <div style="background:var(--bg-primary);border-radius:.5rem;padding:.875rem;text-align:center;">
                <p style="font-size:1.4rem;font-weight:800;color:#2d7a50;margin:0;">{{ $stlData['facultyCount'] ?? 0 }}</p>
                <p style="font-size:.7rem;color:var(--text-secondary);margin:.2rem 0 0;text-transform:uppercase;letter-spacing:.04em;">Team Faculty</p>
            </div>
            <div style="background:var(--bg-primary);border-radius:.5rem;padding:.875rem;text-align:center;">
                <p style="font-size:1.4rem;font-weight:800;color:#2d7a50;margin:0;">{{ $stlData['loadCount'] ?? 0 }}</p>
                <p style="font-size:.7rem;color:var(--text-secondary);margin:.2rem 0 0;text-transform:uppercase;letter-spacing:.04em;">Active Loads</p>
            </div>
            <div style="background:var(--bg-primary);border-radius:.5rem;padding:.875rem;text-align:center;">
                <p style="font-size:1.4rem;font-weight:800;color:#2d7a50;margin:0;">{{ $stlData['scheduleCount'] ?? 0 }}</p>
                <p style="font-size:.7rem;color:var(--text-secondary);margin:.2rem 0 0;text-transform:uppercase;letter-spacing:.04em;">Team Schedules</p>
            </div>
            <div style="background:var(--bg-primary);border-radius:.5rem;padding:.875rem;text-align:center;">
                <p style="font-size:1.4rem;font-weight:800;color:#f59e0b;margin:0;">{{ $stlData['pendingReviews'] ?? 0 }}</p>
                <p style="font-size:.7rem;color:var(--text-secondary);margin:.2rem 0 0;text-transform:uppercase;letter-spacing:.04em;">Pending Reviews</p>
            </div>
        </div>

        @if(!empty($stlData['teamMembers']))
        <div style="overflow-x:auto;border-radius:.5rem;border:1px solid var(--border-color);">
            <table style="width:100%;border-collapse:collapse;">
                <thead>
                    <tr style="background:var(--bg-primary);font-size:.73rem;text-transform:uppercase;letter-spacing:.04em;color:var(--text-secondary);">
                        <th style="padding:.65rem 1rem;text-align:left;font-weight:600;">Faculty Name</th>
                        <th style="padding:.65rem 1rem;text-align:left;font-weight:600;">Email</th>
                        <th style="padding:.65rem 1rem;text-align:left;font-weight:600;">Status</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($stlData['teamMembers'] as $member)
                    <tr style="border-top:1px solid var(--border-color);font-size:.875rem;">
                        <td style="padding:.7rem 1rem;font-weight:600;color:var(--text-primary);">{{ trim(($member->first_name ?? '') . ' ' . ($member->last_name ?? '')) ?: '—' }}</td>
                        <td style="padding:.7rem 1rem;color:var(--text-secondary);">{{ $member->email ?? '—' }}</td>
                        <td style="padding:.7rem 1rem;"><span style="padding:.2rem .6rem;border-radius:2rem;font-size:.7rem;font-weight:700;background:#dcfce7;color:#166534;">Active</span></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        @if(!empty($stlData['dssRecommendations']))
        <div style="margin-top:1.25rem;">
            <h4 style="font-size:.85rem;font-weight:700;color:var(--text-primary);margin:0 0 .75rem;">DSS Recommendations</h4>
            <div style="display:flex;flex-direction:column;gap:.5rem;">
                @foreach($stlData['dssRecommendations'] as $rec)
                <div style="padding:.875rem;background:rgba(2,132,199,.06);border-left:3px solid #0284c7;border-radius:.375rem;">
                    <p style="color:#0c4a6e;font-size:.875rem;font-weight:500;margin:0;">{{ $rec['message'] ?? '' }}</p>
                    <p style="color:#0284c7;font-size:.73rem;margin:.25rem 0 0;">Type: {{ ucfirst($rec['type'] ?? '') }}</p>
                </div>
                @endforeach
            </div>
        </div>
