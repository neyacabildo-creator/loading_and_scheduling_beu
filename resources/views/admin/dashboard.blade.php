...
@section('content')
    ...
    <div class="table-card">
        <div class="table-header">
            ...
            <button class="action-btn action-btn-primary" onclick="openAddFacultyLoadModal()" style="padding: 0.75rem 1.5rem;">+ Add Faculty Load</button>
        </div>
        <div class="table-tabs">
    ...
    <div id="addFacultyLoadModal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <form id="addFacultyLoadForm">
                ...
                <div class="form-group">
                    <label>Notes</label>
                    <textarea id="facultyNotes" placeholder="Additional notes" style="width: 100%; padding: 0.75rem; border: 1px solid #ccc; border-radius: 0.375rem; font-family: inherit;"></textarea>
                </div>
                <div class="form-actions">
    ...
    <script>
        ...
        document.getElementById('addRoomForm')?.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showError(errorMsg || 'Error adding room');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showError(error.message || error.errors?.room_number?.[0] || 'Error adding room');
            });
        });
        ...
        function openAddFacultyLoadModal() {
            ...
            document.getElementById('addFacultyLoadModal').style.display = 'block';
            // Load teachers for dropdown
            loadTeachersForSelect();
        }

        function closeAddFacultyLoadModal() {
        ...
        document.getElementById('addFacultyLoadForm')?.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    showSuccess('Faculty load added successfully');
                    closeAddFacultyLoadModal();
                    loadFacultyLoads();
                } else {
                    const errorMsg = data.errors ? Object.values(data.errors).flat().join(', ') : data.message;
                    showError(errorMsg || 'Error adding faculty load');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showError(error.message || 'Error adding faculty load');
            });
        });
